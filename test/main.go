...
func testStrategyModule() {
	...
	fmt.Printf("Testing Strategy Mode 2 (追涨杀跌2) for stock %s\n", stockCode)
	operates2 := stockStrategy.DealStrategys(stockCode, stockStrategy.Strategy_Mode_2)
	fmt.Printf("Generated %d trade operations\n", len(operates2))
	
	fmt.Printf("Testing Strategy Mode 3 (波动策略) for stock %s\n", stockCode)
	operates3 := stockStrategy.DealStrategys(stockCode, stockStrategy.Strategy_Mode_3)
	fmt.Printf("Generated %d trade operations\n", len(operates3))
