...
func Test_line(t *testing.T) {
	...
	series := chart.ContinuousSeries{
		...
		YValues: yValues,
		Style: chart.Style{
			StrokeColor: drawing.ColorBlack, // 设置折线的颜色
			StrokeWidth: 4,
			DotColor:    drawing.ColorBlue, // 设置数据点的颜色
			DotWidth:    5,
		},
	}
...
func Test_paint(t *testing.T) {
	...
	var xv []time.Time
	var yv []float64
	for _, value := range stock.Datas.DayDatas {
		t, err := time.Parse("2006-01-02", value.DataStr)
		if err != nil {
			// 处理转换错误
			fmt.Printf("parse time:%s failed\n", value.DataStr)

			continue
		}

		xv = append(xv, t)
		yv = append(yv, float64(value.PriceA))
		//if index == 10 {
		//	fmt.Printf("index == 10")
		//	break
		//}
	...
	graph := chart.Chart{
		...
		Width: len(stock.Datas.DayDatas) * 30,
		//Height:
	}
